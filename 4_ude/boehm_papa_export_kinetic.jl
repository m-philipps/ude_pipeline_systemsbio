function ude_dynamics_full!(
    ...
    STAT5A, STAT5B, pApA, pApB, pBpB, _, nucpApB, nucpBpB = @inbounds u[1:state_length]
    ...
    du_mech[1] =
        ...
        v_nuc / v_cyt * k_exp_hetero * nucpApB # + 2 * v_nuc/v_cyt * k_exp_homo * nucpApA 
    # STAT5B
    du_mech[2] =
    ...
    du_nn = nn_model(u[state_idx_nn_in], p.nn, st_nn)[1][1]
    # zero contribution to all states but state_idx_nn_out
    du_nn_full = eltype(u).(zeros(state_length))
    # implement mass conservation
    du_nn_full[state_idx_nn_out[1]] = 2 * v_nuc / v_cyt * du_nn
    du_nn_full[state_idx_nn_out[2]] = -du_nn
