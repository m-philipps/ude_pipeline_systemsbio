...
fnn_depths = latin_hypercube(
    n_samples, lb=np.array(hp_options["fnn_depth"][0]), ub=np.array(hp_options["fnn_depth"][-1] + 1)
...
lrs_adam = latin_hypercube(
    n_samples, lb=np.array(np.log(hp_options["lr_adam"][0])), ub=np.array(np.log(hp_options["lr_adam"][-1]))
).reshape(1, -1)[0]
